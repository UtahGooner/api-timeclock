...
async function loadClockActionEmployee(loginCode:string):Promise<Employee> {
    try {
        if (!loginCode) {
...
async function clockOutHandler(loginCode:string, options:EntryOptions):Promise<ClockActionResult> {
    try {
        ...
        const employee = await loadClockActionEmployee(loginCode);
        let existing:Entry|null = options?.idEntry
...
export const postClockIn = async (req:Request, res:Response) => {
    try {
        const {loginCode, override} = req.body as ClockActionBody;
...
export const postClockOut = async (req:Request, res:Response) => {
    try {
        const {loginCode, override, idEntry, notes} = req.body as ClockActionBody;
...
export const postAdjustClock = async (req:Request, res:Response) => {
    try {
