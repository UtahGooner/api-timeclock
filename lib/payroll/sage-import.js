...
async function parsePayPeriodImportEntries(idPayPeriod) {
    try {
        ...
        const employees = {};
        rows.forEach(row => {
            const { EmployeeKey, Department, EmployeeNumber, PayMethod, DayWorked, WeekWorked, EarningsCode, EarningsType, seconds } = row;
...
export async function testPRImportFile(req, res) {
    try {
...
export async function getPRImportFile(req, res) {
    try {
