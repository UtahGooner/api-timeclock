...
import { addResultToExcelSheet, buildWorkBook, parseDataForAOA } from "chums-local-modules";
...
const formattedSeconds = (seconds = 0, format = 'hhmm') => {
...
export async function parsePayPeriodSSEntries({ idPayPeriod, format = 'hhmm' }) {
    try {
        const emptyPPEarnings = {
            Hours_000001: 0,
            Hours_000002: 0,
            Hours_000003: 0,
            Hours_000004: 0,
        ...
        const rows = await loadPayPeriodSSEntries(idPayPeriod);
        const employees = {};
        rows.forEach(row => {
            const { EmployeeKey, EmployeeName, Department, EmployeeNumber, PayMethod, EmployeeApprovalTime, SupervisorApprovalTime, SupervisorName } = row;
...
export async function getPayPeriodSSData(req, res) {
    try {
...
export async function getPayPeriodSpreadSheet(req, res) {
    try {
        const [payPeriod] = await loadPayPeriods(req.params.idPayPeriod);
        if (!payPeriod) {
